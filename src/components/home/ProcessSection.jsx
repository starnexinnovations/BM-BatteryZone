...
export default function ProcessSection() {
  ...
  return (
    <section className="section-padding bg-dark-card relative overflow-hidden" id="process" aria-label="How we work">
      <div className="container-max relative z-10" ref={ref}>
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="text-center mb-14">
          ...
          <p className="text-slate-400 text-lg max-w-2xl mx-auto">Getting your power solution is quick and hassle-free. Just 4 simple steps.</p>
        </motion.div>
          ...
          {steps.map((step, index) => {
            ...
            return (
              <motion.div key={step.step} initial={{ opacity: 0, y: 40 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: index * 0.15, duration: 0.6 }}
                ...
                <div className={`absolute -top-4 left-1/2 -translate-x-1/2 w-8 h-8 ${colors.bg} rounded-xl flex items-center justify-center text-dark-bg font-display font-black text-xs shadow-lg`}>
                  {step.step}
                </div>
