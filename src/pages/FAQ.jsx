    ...
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.04 }}
      ...
      <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between p-6 text-left group" id={`faq-${faq.id}`} aria-expanded={isOpen}>
        <div className="flex items-start gap-4 flex-1">
          ...
          <h3 className={`font-semibold text-sm md:text-base transition-colors ${isOpen ? 'text-accent' : 'theme-text group-hover:text-accent'}`}>{faq.question}</h3>
        </div>
        <motion.div animate={{ rotate: isOpen ? 180 : 0 }} transition={{ duration: 0.2 }}
          className={`ml-4 flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${isOpen ? 'text-accent bg-accent/10' : 'theme-text-muted'}`}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }}>
            <div className="px-6 pb-6 pt-4" style={{ borderTop: '1px solid var(--card-glass-border)' }}>
              <p className="leading-relaxed text-sm pl-[4.5rem] theme-text-secondary">{faq.answer}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
    ...
    <>
      ...
      <div className="page-header">
        <div className="container-max text-center relative z-10">
          ...
          <p className="max-w-2xl mx-auto theme-text-secondary">Find answers to the most common questions about our products and services.</p>
        </div>
      </div>
      <main className="section-padding relative" style={{ background: 'var(--color-bg)' }}>
        ...
        <div className="container-max max-w-4xl relative z-10">
          <div className="relative mb-8">
            <FiSearch className="absolute left-5 top-1/2 -translate-y-1/2 theme-text-muted" size={18} />
            ...
            {faqCategories.map((cat) => (
              <button key={cat} onClick={() => setActiveCategory(cat)} id={`faq-cat-${cat}`}
                className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-300 ${
                  activeCategory === cat ? 'bg-gradient-to-r from-accent to-accent-dark shadow-glow-amber' : 'glass-card theme-text-secondary hover:text-accent hover:border-accent/30'
                }`}
                style={activeCategory === cat ? { color: '#0A1628' } : {}}>{cat}</button>
            ))}
          </div>
          <div className="space-y-3">
            {filtered.length > 0 ? filtered.map((faq, i) => <FAQItem key={faq.id} faq={faq} index={i} />) : (
              <div className="text-center py-16 glass-card rounded-2xl">
                ...
                <p className="theme-text-secondary">No questions found. Try a different search.</p>
              </div>
            )}
          </div>
          <div className="mt-16 glass-card p-10 rounded-2xl text-center gradient-border">
            <div className="w-16 h-16 mx-auto mb-4 bg-accent/10 border border-accent/20 rounded-2xl flex items-center justify-center"><span className="text-3xl">💬</span></div>
            <h3 className="font-display font-bold text-2xl theme-text mb-3">Still have questions?</h3>
            <p className="mb-6 max-w-lg mx-auto theme-text-secondary">Our experts are ready to help. Chat with us on WhatsApp for instant answers.</p>
