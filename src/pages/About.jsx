    ...
    <>
      ...
      <main className="bg-dark-bg">
          ...
          <div className="container-max">
            <div className="grid lg:grid-cols-2 gap-16 items-center">
              <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
                <h2 className="text-3xl font-display font-bold text-white mb-6">Coimbatore's Most Trusted <span className="gradient-text">Power Solutions</span> Provider</h2>
              ...
              <div className="space-y-8">
                {milestones.map((m, i) => (
                  <motion.div key={m.year} initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
                    ...
                    <div className="w-16 h-16 bg-gradient-to-br from-accent to-accent-dark rounded-2xl flex items-center justify-center text-dark-bg font-display font-black text-sm flex-shrink-0 relative z-10 shadow-glow-amber">{m.year}</div>
                    <div className="glass-card p-5 rounded-xl flex-1 hover:border-accent/25 transition-colors"><p className="text-white font-medium">{m.event}</p></div>
                  </motion.div>
                ))}
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {team.map((m, i) => (
                <motion.div key={m.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                  ...
                  <p className="text-slate-500 text-xs">{m.exp} Experience</p>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <section className="py-16 px-4 text-center">
          <h3 className="font-display font-bold text-2xl text-white mb-4">Ready to work with us?</h3>
          <p className="text-slate-400 mb-6">Contact us today for the best battery and solar solutions in Coimbatore.</p>
