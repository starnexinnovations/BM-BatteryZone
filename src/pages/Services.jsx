...
function ServiceDetail({ service, index }) {
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-12 items-center py-16 border-b border-white/[0.05]" ref={ref} id={`service-${service.id}`}>
      <motion.div initial={{ opacity: 0, x: isEven ? -40 : 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.7 }} className={isEven ? '' : 'lg:order-2'}>
        ...
        <div className="mb-8">
          <p className="text-xs text-slate-500 uppercase tracking-wider font-medium mb-3">Available Brands</p>
      ...
      <motion.div initial={{ opacity: 0, x: isEven ? 40 : -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.7, delay: 0.15 }}
        ...
        <div className="text-center">
          ...
          <p className="text-slate-400 text-sm font-medium">{service.title}</p>
        </div>
      </motion.div>
    </div>
  );
    ...
    <>
      ...
      <div className="page-header">
        <div className="container-max text-center relative z-10">
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-slate-400 max-w-2xl mx-auto">
      ...
      <main className="section-padding bg-dark-bg relative">
        ...
        <div className="container-max relative z-10">
          <div className="grid md:grid-cols-3 gap-5 mb-20">
            {services.slice(0, 6).map((s, i) => (
              <motion.div key={s.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
                ...
                <h3 className="font-display font-bold text-white text-base mb-2 group-hover:text-accent transition-colors">{s.title}</h3>
          ...
          <div className="mt-16 rounded-3xl overflow-hidden relative gradient-border">
            <div className="absolute inset-0 bg-gradient-to-r from-electric/20 via-dark-card to-accent/15" />
            <div className="relative z-10 p-10 md:p-14 flex flex-col md:flex-row items-center justify-between gap-8">
              <div>
                ...
                <h3 className="font-display font-bold text-2xl md:text-3xl text-white mb-3">Annual Maintenance Contracts Available</h3>
                <p className="text-slate-400 max-w-xl">Protect your investment with affordable AMC plans for UPS, solar systems, and inverters.</p>
              </div>
